...
public class ClientEvent {
    ...
    public void onPress(InputEvent.KeyInputEvent e) {
        ...
        if (Keyboard.isKeyDown(Keyboard.KEY_TAB) && setBlockMode && mc.inGameHasFocus) {
            if (blockPos != null)
            Main.NETWORK.sendToServer(new CreateBlockServerMessagePacket(blockPos));
            else
                if (mc.world.isRemote)
                mc.player.sendMessage(new TextComponentString("Наведитесь на блок, на который хотите установить декор"));
        }
    }
